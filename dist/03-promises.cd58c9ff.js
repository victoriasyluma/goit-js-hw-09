...
form.addEventListener("submit", async (event)=>{
    ...
    const executePromise = ({ position , delay  })=>{
        return createPromise(position, delay).then(()=>{
            (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${totalDelay}ms`);
        }).catch(()=>{
            (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${totalDelay}ms`);
        }).finally(()=>{
    ...
    for(let position = 2; position <= amountOfSteps; position++)await executePromise({
        position,
        delay: stepsDelayMs
    });
});
